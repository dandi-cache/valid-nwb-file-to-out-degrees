...
# every run, so each result records the exact input commit it was computed from. Each valid
# HDF5 asset is then streamed directly from the public DANDI S3 bucket at run time (the
# container is not network-isolated), so no separate input pins that.
...
datalad containers-run -n pipeline --explicit \
  "${RUN_INPUT_ARGS[@]}" \
  --output derivatives \
  -m "Update valid-nwb-file-to-out-degrees (code @ ${GITHUB_SHA}; image ${DIGEST})" \
...
if [ -f "${DS}/derivatives/valid_nwb_file_to_out_degrees.jsonl.gz" ]; then
  cp "${DS}/derivatives/valid_nwb_file_to_out_degrees.jsonl.gz" "${DISTDIR}/derivatives/"
fi
...
git -C "${DISTDIR}" commit -q -m "Publish valid-nwb-file-to-out-degrees"
